...
class OAuthAuthenticationSecurityConfiguration implements WsConfigurer {
	...
	Customizer<HttpSecurity> httpSecurityCustomizer(@Value("${spring.webservices.path}") String wsPath) {
		return httpSecurity -> httpSecurity.authorizeHttpRequests(a -> a.requestMatchers(wsPath).permitAll())
...
class OAuthBinarySecurityTokenEndpointInterceptor implements EndpointInterceptor {
